...
def load_hla_pep_df(folder=r"x:\Feng\HLA-DB\all_alleles\mixmhcpred",rank=2):
    ...
    for fname in os.listdir(folder):
        df = pd.read_table(
            os.path.join(folder, fname),skiprows=11
        )
...
class NonSpecificDigest:
    def __init__(self, protein_data:typing.Tuple[pd.DataFrame,list,str], lens=[8,14]):
        if isinstance(protein_data, pd.DataFrame):
            self.cat_protein_sequence = "$"+"$".join(
                protein_data.sequence.values
            )+"$"
        else:
            ...
            self.cat_protein_sequence = "$"+"$".join([
                _["sequence"] for _ in protein_dict.values()
            ])+"$"
