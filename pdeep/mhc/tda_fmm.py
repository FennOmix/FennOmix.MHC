...
class TDA_fmm(object):
    ...
    def plot(self, title, plot_scores, false_scores = None):
        ...
        if self.external_model is not None and false_scores is not None:
            ...
            n, bins, patches = pp.hist(scores, binsize, density=1, facecolor='green', alpha=0.75)
            ...
            n, bins, patches = pp.hist(scores, binsize, density=1, facecolor='blue', alpha=0.75)
            bins = np.arange(scores.min(), scores.max(), step)
            dis = self.pdf(bins)
...
class DecoyModel(TDA_fmm):
    ...
    def fit(self, X):
        ...
        if self.guassian_outlier_sigma:
            X = X[
                X>self.mu-self.guassian_outlier_sigma*self.sigma
            ]
            self.mu = np.mean(X)
            self.sigma = np.std(X)
...
def select_best_fmm(target_scores, decoy_fmm, _max_component_ = 3, verbose=True):
    ...
    for n_component in range(1, _max_component_+1):
        ...
        if BIC < best_target_BIC:
            best_target_BIC = BIC
            best_fmm = target_fmm
        if verbose: print("[Target FMM] G=%d, BIC=%f, best BIC=%f"%(n_component, BIC, best_target_BIC))
    return best_fmm
