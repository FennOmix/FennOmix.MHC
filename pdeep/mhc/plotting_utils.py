...
def plot_umap_df(
    ...
    for i,factor in enumerate(factors):
        if jump_color:
            ...
            color_mapping[factor] = Turbo256[int(i*1.0*len(Turbo256)/len(factors))]
    fig = px.scatter(
        df, x="UMAP1", y="UMAP2", 
...
def plot_motif_multi_mer(df, allele_col, allele, kmers, axes=None, logo_scale=20):
    ...
    df = df.drop_duplicates([allele_col, 'sequence']+(['mods','mod_sites'] if "mods" in df.columns else []))
    if axes is None:
        fig, axes = plt.subplots(1, len(kmers), sharey='row')
    logo_plots = []
    max_y_vals = []
    for i,ax in enumerate([axes] if len(kmers)==1 else axes):
        logo_plots.append(plot_motif(df, allele_col, allele, kmers[i], ax=ax, logo_scale=logo_scale))
        max_y_vals.append(np.ceil(ax.get_ylim()[1]*5+1)*0.2)
        
    max_y = np.max(max_y_vals)
    for i,logos in enumerate(logo_plots):
        logos.ax.text(3, max_y+0.1, f"n={len(df[(df[allele_col]==allele)&(df.nAA==kmers[i])])}")
